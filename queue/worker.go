	...
	go func() {
		for {
			...
			case job := <-w.JobChannel:
				...
				JobsProcessed.WithLabelValues(w.Name, MetricType).Inc()
				if err := job.Handle(); err != nil {
					log.Println("Error in job: %s", err.Error())
				}

				RunningJobs.WithLabelValues(MetricType).Dec()
				ProcessingTime.WithLabelValues(w.Name, MetricType).Observe(time.Now().Sub(startTime).Seconds())
...
func (w Worker) Stop() {
	RunningWorkers.WithLabelValues(MetricType).Dec()
	go func() {
		w.quit <- true
	}()
}